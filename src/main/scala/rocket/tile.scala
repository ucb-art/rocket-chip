...
class RocketTile(implicit p: Parameters) extends LazyModule {
  ...
  val icLegacy = LazyModule(new TLLegacy()(p))
  val roccLegacy = LazyModule(new TLLegacy()(p))

  (slave zip scratch) foreach { case (node, lm) => lm.node := TLFragmenter(p(XLen)/8, p(CacheBlockBytes))(node) }
  
  lazy val module = new LazyModuleImp(this) {
    val io = new Bundle {
      val dcache = dcache.node.bundleOut
      val icache = icLegacy.node.bundleOut
      val rocc = roccLegacy.node.bundleOut
      val slave = slave.map(_.bundleIn)
    ...
    icLegacy.module.io.legacy <> icache.io.mem
    ...
    roccLegacy.module.io.legacy <> uncachedArb.io.out
