...
class DCache(implicit p: Parameters) extends LazyModule with HasL1CacheParameters {
  val node = TLClientNode(TLClientParameters(supportsProbe = TransferSizes(p(CacheBlockBytes))))
